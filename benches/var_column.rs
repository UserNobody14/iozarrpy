...
fn bench_fast_path_all(c: &mut Criterion) {
    ...
    c.bench_function(
        "build_var_column/fast_path_all_1000",
        |b| {
            b.iter(|| {
                build_var_column(
                    black_box(&name),
...
fn bench_fast_path_sparse_small(
    c: &mut Criterion,
) {
    let chunk_shape = [10u64, 10, 10];
    let strides = compute_strides(&chunk_shape);
...
fn bench_fast_path_sparse_multivar(
    c: &mut Criterion,
) {
