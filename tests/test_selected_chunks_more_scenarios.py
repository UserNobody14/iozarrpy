...
def test_any_all_horizontal_narrow(baseline_datasets: dict[str, str], zarr_key: str) -> None:
    ...
    chunks, _ = _core._selected_chunks_debug(zarr_url, any_pred, variables=["2m_temperature"])
    idxs = _chunk_indices(chunks)
    expected_count = 2 * per_x if zarr_key == "grid_chunked" else per_x
    assert len(idxs) == expected_count
    assert len(idxs) < all_total
    ...
    chunks, _ = _core._selected_chunks_debug(zarr_url, all_pred, variables=["2m_temperature"])
    idxs = _chunk_indices(chunks)
...
def test_datetime_and_duration_constraints_narrow(baseline_datasets: dict[str, str], zarr_key: str) -> None:
    ...
    chunks, _ = _core._selected_chunks_debug(zarr_url, pred_time, variables=["2m_temperature"])
    idxs = _chunk_indices(chunks)
    ...
    chunks, _ = _core._selected_chunks_debug(zarr_url, pred_lt, variables=["2m_temperature"])
...
def test_negation_cuts_hole_in_selected_cube(baseline_datasets: dict[str, str]) -> None:
    ...
    chunks, _ = _core._selected_chunks_debug(zarr_url, pred, variables=["geopotential_height"])
