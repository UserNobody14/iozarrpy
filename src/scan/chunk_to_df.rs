...
pub(crate) async fn chunk_to_df_tree(
    ...
    if !meta.is_hierarchical() {
        ...
        let dims = Arc::new(meta.dim_analysis.all_dims.clone());
        let vars = Arc::new(meta.root.data_vars.clone());
        return chunk_to_df(
            idx,
            primary,
            Arc::new(legacy_meta),
    ...
    for (d, dim_name) in output_dims.iter().enumerate() {
        if !should_emit_col(dim_name, &with_columns) {
            continue;
        }
        ...
        let coord_data = if let Some((_, arr)) = coord_arrays.iter().find(|(n, _)| n == dim_name) {
            if d < origin.len() {
                let dim_start = origin[d];
                let dim_len = chunk_shape[d];
                retrieve_1d_subset_async(arr, dim_start, dim_len)
                    .await
                    .ok()
            } else {
                None
    ...
    for var_name in &meta.root.data_vars {
        if !should_emit_col(var_name, &with_columns) {
            continue;
        }

        let Some((_, arr)) = var_arrays.iter().find(|(n, _)| n == var_name) else {
            continue;
        };

        let Some(var_meta) = meta.root.arrays.get(var_name) else {
            continue;
        };

        let data = retrieve_chunk_async(arr, &idx).await.map_err(to_py_err)?;
        let var_dims: Vec<IStr> = var_meta.dims.iter().cloned().collect();
    ...
    for (child_name, child_node) in &meta.root.children {
        if !should_emit_group(child_name, &with_columns) {
            continue;
        }

        let struct_col = build_group_struct_column(
            child_name,
            child_node,
            &idx,
            output_dims,
...
fn should_emit_col(name: &IStr, with_columns: &Option<BTreeSet<IStr>>) -> bool {
    with_columns
        .as_ref()
        .map(|s| s.contains(name))
        .unwrap_or(true)
...
fn should_emit_group(name: &IStr, with_columns: &Option<BTreeSet<IStr>>) -> bool {
        ...
        .map(|s| {
            s.contains(name)
                || s.iter().any(|v| {
                    ...
                    { let name_str: &str = name.as_ref(); v_str.starts_with(&format!("{}/", name_str)) }
                })
        })
        .unwrap_or(true)
}
...
fn build_dim_column_internal(
    ...
    if let Some(te) = time_encoding {
        let mut out_i64: Vec<i64> = Vec::with_capacity(keep.len());
        for &row in keep {
            let local = (row as u64 / strides[dim_idx]) % chunk_shape[dim_idx];
            let raw_value = coord_data
                .and_then(|c| c.get_i64(local as usize))
                .unwrap_or((origin[dim_idx] + local) as i64);
            let ns = if te.is_duration {
                raw_value.saturating_mul(te.unit_ns)
            } else {
                raw_value
        ...
        if coord.is_float() {
            ...
            Series::new(dim_name.into(), out_f64).into()
        } else {
            let mut out_i64: Vec<i64> = Vec::with_capacity(keep.len());
            for &row in keep {
                let local = (row as u64 / strides[dim_idx]) % chunk_shape[dim_idx];
            ...
            Series::new(dim_name.into(), out_i64).into()
        }
    } else {
        let mut out_i64: Vec<i64> = Vec::with_capacity(keep.len());
        for &row in keep {
            let local = (row as u64 / strides[dim_idx]) % chunk_shape[dim_idx];
            out_i64.push((origin[dim_idx] + local) as i64);
        }
        Series::new(dim_name.into(), out_i64).into()
    }
}
...
fn build_var_column_with_broadcast(
    ...
    let same_dims = var_dims.len() == output_dims.len()
        && var_dims.iter().zip(output_dims.iter()).all(|(a, b)| a == b);

    if same_dims {
        data.take_indices(keep).into_series(var_name).into()
    } else {
        // Need to broadcast: map output indices to variable indices
        let dim_mapping: Vec<Option<usize>> = output_dims
            .iter()
            .map(|od| var_dims.iter().position(|vd| vd == od))
            .collect();
        let var_strides = compute_strides(var_chunk_shape);
        ...
        let indices: Vec<usize> = keep
            ...
            .map(|&row| {
                let mut var_idx: u64 = 0;
                for (out_d, maybe_var_d) in dim_mapping.iter().enumerate() {
                    if let Some(var_d) = *maybe_var_d {
                        let local = (row as u64 / output_strides[out_d]) % output_chunk_shape[out_d];
                        if var_d < var_strides.len() {
                            var_idx += local * var_strides[var_d];
                        }
                    }
                }
                var_idx as usize
        ...
        data.take_indices(&indices).into_series(var_name).into()
    }
}
...
async fn build_group_struct_column(
    ...
    for var_name in &node.data_vars {
        ...
        let node_path_str: &str = node.path.as_ref();
        let var_path = format!("{}/{}", node_path_str.trim_start_matches('/'), var_name);

        let arr_opt = var_arrays
            .iter()
            .find(|(n, _)| { 
            let n_str: &str = n.as_ref(); 
            let var_str: &str = var_name.as_ref();
            n_str == var_path || n_str == var_str 
        });

        if let Some((_, arr)) = arr_opt {
            let var_meta = node.arrays.get(var_name);
            ...
            let var_chunk_shape: Vec<u64> = arr
                .chunk_shape(idx)
                .map(|s| s.iter().map(|x| x.get()).collect())
                .unwrap_or_default();

            // Build the series with broadcasting
            let series = build_var_series_with_broadcast(
                var_name.as_ref(),
                &data,
                &var_dims,
                output_dims,
    ...
    if field_series.is_empty() {
        // Empty struct
        let null_series = Series::new_null(group_name_str.into(), keep.len());
        Ok(null_series.into())
    } else {
        let struct_chunked = StructChunked::from_series(group_name_str.into(), field_series.len(), field_series.iter())
...
fn build_var_series_with_broadcast(
    ...
    if same_dims {
        data.take_indices(keep).into_series(var_name)
    } else {
        // Need to broadcast: map output indices to variable indices
        let dim_mapping: Vec<Option<usize>> = output_dims
            .iter()
        ...
        data.take_indices(&indices).into_series(var_name)
    }
}
