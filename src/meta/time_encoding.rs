...
pub(crate) fn extract_time_encoding<TStorage: ?Sized>(array: &Array<TStorage>) -> Option<TimeEncoding> {
        ...
        if let Some(dtype_unit_ns) = parse_timedelta_dtype(&dtype_str) {
            if let Some(units_str) = units.as_deref() {
                if !units_str.contains(" since ") {
                    if let Some(unit_ns) = parse_duration_units(units_str) {
    ...
    if let Some(units_str) = units.as_deref() {
        if let Some((unit_ns, epoch_ns)) = parse_cf_time_units(units_str) {
            return Some(TimeEncoding {
                epoch_ns,
                unit_ns,
                is_duration: false,
...
fn parse_timedelta_dtype(dtype_str: &str) -> Option<i64> {
    ...
    if !s.starts_with("timedelta64[") || !s.ends_with(']') {
        return None;
    }
    let unit = s.trim_start_matches("timedelta64[").trim_end_matches(']');
    match unit {
        "ns" => Some(1),
        "us" | "µs" => Some(1_000),
        "ms" => Some(1_000_000),
