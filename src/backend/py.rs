...
impl PyZarrBackend {
    ...
    fn from_url(url: String, max_cache_entries: usize) -> PyResult<Self> {
        let opened = crate::store::open_store_async(&url)
            .map_err(|e| PyErr::new::<pyo3::exceptions::PyValueError, _>(e))?;

        let backend = CachingAsyncBackend::new(opened.store, opened.root, max_cache_entries);
    ...
    fn scan_zarr_async<'py>(
        ...
        let with_columns_set: Option<BTreeSet<String>> =
            with_columns.map(|v| v.into_iter().collect::<BTreeSet<_>>());

        let backend = self.inner.clone();

        future_into_py(py, async move {
            let df = scan_zarr_with_backend_async(
                backend,
                expr,
                variables,
                max_concurrency,
    ...
    fn schema(&self, variables: Option<Vec<String>>) -> PyResult<PySchema> {
        use crate::IntoIStr;
        ...
        let vars: Option<Vec<crate::IStr>> =
            variables.map(|v| v.into_iter().map(|s| s.istr()).collect());
        let schema = meta.tidy_schema(vars.as_deref());
    ...
    fn clear_coord_cache<'py>(&self, py: Python<'py>) -> PyResult<Bound<'py, PyAny>> {
        let backend = self.inner.clone();
        future_into_py(py, async move {
    ...
    fn clear_all_caches<'py>(&self, py: Python<'py>) -> PyResult<Bound<'py, PyAny>> {
        let backend = self.inner.clone();
        future_into_py(py, async move {
    ...
    fn cache_stats<'py>(&self, py: Python<'py>) -> PyResult<Bound<'py, PyAny>> {
        let backend = self.inner.clone();
        future_into_py(py, async move {
            let stats = backend.cache_stats().await;
            Python::attach(|py| {
                let dict = pyo3::types::PyDict::new(py);
                dict.set_item("coord_entries", stats.coord_entries)?;
                dict.set_item("has_metadata", stats.has_metadata)?;
    ...
    fn __repr__(&self) -> String {
        format!("ZarrBackend(root='{}')", self.inner.root())
    }
}
...
async fn scan_zarr_with_backend_async(
    ...
    let semaphore = StdArc::new(tokio::sync::Semaphore::new(max_conc));
    ...
    let with_columns: StdArc<Option<BTreeSet<crate::IStr>>> = StdArc::new(
    ...
    for idx in indices {
        ...
        let var_arrays = StdArc::clone(&var_arrays);
        let coord_arrays = StdArc::clone(&coord_arrays);
        let with_columns = StdArc::clone(&with_columns);
    ...
    let mut out: Option<polars::prelude::DataFrame> = None;
    ...
    Ok(out.unwrap_or_else(|| {
        polars::prelude::DataFrame::empty_with_schema(&meta.tidy_schema(Some(&vars)))
    }))
}
