...
use super::super::compile_node::{collect_column_refs, extract_struct_field_path};
use super::super::expr_plan::{ExprPlan, VarSet};
use super::super::expr_utils::try_expr_to_value_range_lazy;
use super::super::literals::{col_lit, literal_anyvalue, reverse_operator, strip_wrappers};
use super::boolean::compile_boolean_function_lazy;
use super::cmp::{compile_cmp_to_plan, compile_struct_field_cmp, compile_value_range_to_plan};
use super::interpolate::interpolate_selection_nd_lazy;
use super::selector::compile_selector_lazy;
use super::utils::{collect_refs_from_expr, refs_to_plan, refs_to_plan_with_vars};
...
pub(crate) fn compile_expr(
    ...
    match expr {
        Expr::Display { .. } => {
            panic!("Display expression not supported");
        }
        Expr::Alias(inner, _) => compile_expr(inner.as_ref(), ctx),
        Expr::KeepName(inner) => compile_expr(inner.as_ref(), ctx),
        Expr::RenameAlias { expr, .. } => compile_expr(expr.as_ref(), ctx),
        Expr::Cast { expr, .. } => compile_expr(expr.as_ref(), ctx),
        Expr::Sort { expr, .. } => compile_expr(expr.as_ref(), ctx),
        Expr::SortBy { expr, .. } => compile_expr(expr.as_ref(), ctx),
        Expr::Explode { input, .. } => compile_expr(input.as_ref(), ctx),
        Expr::Slice { input, .. } => compile_expr(input.as_ref(), ctx),
        ...
        Expr::Over {
            ...
            let func_plan = compile_expr(function.as_ref(), ctx)?;
            ...
            if refs.is_empty() {
                ...
                Ok(func_plan.add_vars(VarSet::from_vec(refs)))
            }
        }

        Expr::Rolling { function, .. } => compile_expr(function.as_ref(), ctx),

        Expr::Filter { input, by } => {
            let filter_plan = compile_expr(by.as_ref(), ctx)?;
            if filter_plan.is_empty() {
                return Ok(ExprPlan::Empty);
            }
            let input_plan = compile_expr(input.as_ref(), ctx)?;
            let result = input_plan.intersect(&filter_plan);
            let filter_refs: Vec<IStr> = collect_refs_from_expr(by.as_ref())
                .into_iter()
                .filter(|r| ctx.meta.array_by_path(r.clone()).is_some())
                .collect();
            if filter_refs.is_empty() {
                Ok(result)
            } else {
                Ok(result.add_vars(VarSet::from_vec(filter_refs)))
            }
        }
        ...
        Expr::BinaryExpr { left, op, right } => {
            match op {
                Operator::And | Operator::LogicalAnd => {
                    ...
                    let a = compile_expr(left.as_ref(), ctx)?;
                    let b = compile_expr(right.as_ref(), ctx)?;
                    Ok(a.intersect(&b))
                }
                ...
                Operator::Eq | Operator::GtEq | Operator::Gt | Operator::LtEq | Operator::Lt => {
                    ...
                    if let Some((struct_col, field_name)) =
                        extract_struct_field_path(strip_wrappers(left.as_ref()))
                    {
                    ...
                    if let Some((struct_col, field_name)) =
                        extract_struct_field_path(strip_wrappers(right.as_ref()))
                    {
                    ...
                    if let Some((col, lit)) =
                        col_lit(left, right).or_else(|| col_lit(right, left))
                    {
                ...
                _ => Ok(refs_to_plan(collect_refs_from_expr(expr))),
            }
        }

        Expr::Literal(lit) => match literal_anyvalue(lit) {
            Some(AnyValue::Boolean(true)) => Ok(ExprPlan::NoConstraint),
            Some(AnyValue::Boolean(false)) => Ok(ExprPlan::Empty),
            Some(AnyValue::Null) => Ok(ExprPlan::Empty),
        ...
        Expr::Ternary {
            ...
            if let (Expr::Literal(t), Expr::Literal(f)) = (
                ...
                if let (Some(AnyValue::Boolean(t)), Some(AnyValue::Boolean(f))) = (
                    ...
                    if t && !f {
                        return compile_expr(predicate.as_ref(), ctx);
                    }
                    if f {
            ...
            let all_refs = collect_refs_from_expr(expr);
            ...
            if is_falsy_null {
                let predicate_plan = compile_expr(predicate.as_ref(), ctx)?;
        ...
        Expr::Function { input, function } => match function {
            ...
            FunctionExpr::NullCount => Ok(ExprPlan::NoConstraint),
        ...
        Expr::Column(name) => Ok(ExprPlan::unconstrained_vars(VarSet::single(name.istr()))),
        ...
        Expr::Eval { expr, .. } => compile_expr(expr.as_ref(), ctx),
        ...
        | Expr::Len
        | Expr::SubPlan(_, _)
        | Expr::DataTypeFunction(_) => Ok(ExprPlan::NoConstraint),

        Expr::StructEval { .. } => Ok(ExprPlan::NoConstraint),
    }
}
