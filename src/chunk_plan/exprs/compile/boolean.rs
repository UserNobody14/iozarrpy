...
use super::super::literals::{literal_anyvalue, strip_wrappers};
...
pub(super) fn compile_boolean_function_lazy(
    ...
    match bf {
        BooleanFunction::Not => {
            ...
            if let Expr::Literal(lit) = strip_wrappers(arg) {
                return match literal_anyvalue(lit) {
                    Some(AnyValue::Boolean(true)) => Ok(ExprPlan::Empty),
                    Some(AnyValue::Boolean(false)) => Ok(ExprPlan::NoConstraint),
                    Some(AnyValue::Null) => Ok(ExprPlan::Empty),
                    _ => Ok(ExprPlan::NoConstraint),
                };
            }
            let inner = compile_expr(arg, ctx).unwrap_or(ExprPlan::NoConstraint);
            if inner.is_empty() {
        ...
        BooleanFunction::AnyHorizontal => {
            ...
            for e in input {
                let plan = compile_expr(e, ctx).unwrap_or(ExprPlan::NoConstraint);
                acc = acc.union(&plan);
            }
            Ok(acc)
        }
...
pub(super) fn compile_is_between_lazy(
    ...
    let a = compile_cmp_to_plan(&col, Operator::GtEq, low_lit, ctx)
        .unwrap_or(ExprPlan::NoConstraint);
    let b = compile_cmp_to_plan(&col, Operator::LtEq, high_lit, ctx)
        .unwrap_or(ExprPlan::NoConstraint);
    Ok(a.intersect(&b))
}
...
pub(super) fn compile_is_in_lazy(
    ...
    for av in values {
        ...
        let node = compile_cmp_to_plan(&col, Operator::Eq, &lit, ctx)
            .unwrap_or(ExprPlan::NoConstraint);
