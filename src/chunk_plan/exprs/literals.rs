...
pub(super) fn literal_anyvalue(
    lit: &LiteralValue,
) -> Option<AnyValue<'static>> {
    match lit {
        LiteralValue::Scalar(s) => Some(
            s.clone().into_value().into_static(),
        ),
        ...
        other => {
            ...
            if cfg!(debug_assertions) {
                eprintln!(
                    "chunk_plan: unsupported LiteralValue variant for planning: {other:?}"
                );
            }
            None
        }
...
pub(super) fn literal_to_scalar(
    ...
    let parse_temporal_from_str = |s: &str| -> Option<
        CoordScalar,
    > {
        let s = s.trim();
        ...
        if let Some(hours) =
            s.strip_suffix('h').and_then(|v| {
                v.trim().parse::<i64>().ok()
            })
        {
            return Some(
                CoordScalar::DurationNs(
                    hours.saturating_mul(
                        3_600_000_000_000,
                    ),
                ),
            );
    ...
    match lit {
        ...
        _ => {
            let parsed = match literal_anyvalue(
                lit,
            )? {
                AnyValue::Int64(v) => {
                    Some(CoordScalar::I64(v))
                }
                AnyValue::Int32(v) => Some(
                    CoordScalar::I64(v as i64),
                ),
                AnyValue::Int16(v) => Some(
                    CoordScalar::I64(v as i64),
                ),
                AnyValue::Int8(v) => Some(
                    CoordScalar::I64(v as i64),
                ),
                AnyValue::UInt64(v) => {
                    Some(CoordScalar::U64(v))
                }
                AnyValue::UInt32(v) => Some(
                    CoordScalar::U64(v as u64),
                ),
                AnyValue::UInt16(v) => Some(
                    CoordScalar::U64(v as u64),
                ),
                AnyValue::UInt8(v) => Some(
                    CoordScalar::U64(v as u64),
                ),
                AnyValue::Float64(v) => {
            ...
            parsed.or_else(|| {
                parse_temporal_from_str(&format!(
                    "{lit:?}"
                ))
            })
        }
