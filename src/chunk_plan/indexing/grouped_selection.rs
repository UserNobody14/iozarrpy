...
use super::selection::{Emptyable, SetOperations};
...
pub trait ArraySelectionType: SetOperations + Clone {
...
impl<Sel: ArraySelectionType> GroupedSelection<Sel> {
    ...
    pub fn for_vars_with_selection(
        ...
        let mut by_dims: BTreeMap<Arc<DimSignature>, Sel> = BTreeMap::new();
        let mut var_to_sig: BTreeMap<IStr, Arc<DimSignature>> = BTreeMap::new();
        // Cache of signature -> Arc for deduplication
        let mut sig_cache: BTreeMap<DimSignature, Arc<DimSignature>> = BTreeMap::new();
        ...
        for var in vars {
            ...
            by_dims.entry(sig_arc).or_insert_with(|| sel.clone());
        }

        Self { by_dims, var_to_sig }
    }

    /// Create a grouped selection for variables with all indices selected.
    pub fn all_for_vars(vars: impl IntoIterator<Item = IStr>, meta: &ZarrDatasetMeta) -> Self {
        Self::for_vars_with_selection(vars, meta, Sel::all())
    }
    ...
    pub fn get(&self, var: &str) -> Option<&Sel> {
        let sig = self.var_to_sig.get(&crate::IntoIStr::istr(var))?;
    ...
    pub fn vars(&self) -> impl Iterator<Item = (&str, &Sel)> {
        self.var_to_sig.iter().filter_map(|(var, sig)| {
            self.by_dims.get(sig).map(|sel| (var.as_ref(), sel))
        })
    }

    /// Iterate over unique (signature, selection) pairs.
    pub fn by_signature(&self) -> impl Iterator<Item = (&DimSignature, &Sel)> {
        self.by_dims.iter().map(|(sig, sel)| (sig.as_ref(), sel))
    }

    /// Get the dimension signature for a variable.
    pub fn get_signature(&self, var: &str) -> Option<&Arc<DimSignature>> {
        self.var_to_sig.get(&crate::IntoIStr::istr(var))
    }

    /// Get the internal by_dims map (for materialization).
    pub(crate) fn by_dims(&self) -> &BTreeMap<Arc<DimSignature>, Sel> {
        &self.by_dims
    }

    /// Get the internal var_to_sig map (for materialization).
    pub(crate) fn var_to_sig(&self) -> &BTreeMap<IStr, Arc<DimSignature>> {
        &self.var_to_sig
    }
...
impl<Sel: ArraySelectionType> Default for GroupedSelection<Sel> {
    fn default() -> Self {
        Self::new()
    }
}

impl<Sel: ArraySelectionType> Emptyable for GroupedSelection<Sel> {
    fn empty() -> Self {
        Self::new()
    }

    fn is_empty(&self) -> bool {
        self.by_dims.is_empty() || self.by_dims.values().all(|s| s.is_empty())
    }
}

impl<Sel: ArraySelectionType> SetOperations for GroupedSelection<Sel> {
    fn union(&self, other: &Self) -> Self {
        ...
        let mut var_to_sig = self.var_to_sig.clone();

        // Add variables from other
        for (var, sig) in &other.var_to_sig {
            var_to_sig.entry(var.clone()).or_insert_with(|| sig.clone());
        }
        ...
        for (sig, sel) in &other.by_dims {
            by_dims
                .entry(sig.clone())
                .and_modify(|existing| *existing = existing.union(sel))
    ...
    fn intersect(&self, other: &Self) -> Self {
        ...
        for (var, sig) in &self.var_to_sig {
            if other.var_to_sig.contains_key(var) {
                var_to_sig.insert(var.clone(), sig.clone());
            }
        }

        // Intersect selections by signature
        for (sig, sel_a) in &self.by_dims {
            if let Some(sel_b) = other.by_dims.get(sig) {
                let intersected = sel_a.intersect(sel_b);
                if !intersected.is_empty() {
                    by_dims.insert(sig.clone(), intersected);
                }
            }
        }
    ...
    fn difference(&self, other: &Self) -> Self {
        ...
        let mut original_sigs: std::collections::HashSet<Arc<DimSignature>> =
        ...
        for (var, sig) in &self.var_to_sig {
            if !other.var_to_sig.contains_key(var) {
                var_to_sig.insert(var.clone(), sig.clone());
                if let Some(sel) = self.by_dims.get(sig) {
                    by_dims.entry(sig.clone()).or_insert_with(|| sel.clone());
                    original_sigs.insert(sig.clone());
                }
            }
        }
        ...
        for (var, sig) in &self.var_to_sig {
            if other.var_to_sig.contains_key(var) {
                ...
                if let (Some(sel_a), Some(sel_b)) = (self.by_dims.get(sig), other.by_dims.get(sig)) {
                    let diff = sel_a.difference(sel_b);
                    if !diff.is_empty() {
                        var_to_sig.insert(var.clone(), sig.clone());
                        by_dims.entry(sig.clone()).or_insert_with(|| diff);
                    }
...
impl<Sel: ArraySelectionType> Default for DatasetSelectionBase<Sel> {
    fn default() -> Self {
        Self::NoSelectionMade
    }
}

impl<Sel: ArraySelectionType> DatasetSelectionBase<Sel> {
    ...
    /// Iterate over variables and their selections.
    pub fn vars(&self) -> Box<dyn Iterator<Item = (&str, &Sel)> + '_> {
        match self {
            Self::Selection(sel) => Box::new(sel.vars()),
    ...
    pub fn get(&self, var: &str) -> Option<&Sel> {
        match self {
            ...
            Self::NoSelectionMade | Self::Empty => None,
        }
    }

    /// Get the inner grouped selection, if present.
    pub fn as_selection(&self) -> Option<&GroupedSelection<Sel>> {
        match self {
    ...
    pub fn for_vars_with_selection(
        ...
        let grouped = GroupedSelection::for_vars_with_selection(vars, meta, sel);
        if grouped.is_empty() {
            Self::Empty
        } else {
    ...
    pub fn all_for_vars(vars: impl IntoIterator<Item = IStr>, meta: &ZarrDatasetMeta) -> Self {
        let grouped = GroupedSelection::all_for_vars(vars, meta);
        if grouped.is_empty() {
            Self::Empty
        } else {
            Self::Selection(grouped)
...
impl<Sel: ArraySelectionType> Emptyable for DatasetSelectionBase<Sel> {
    fn empty() -> Self {
        Self::Empty
    }
...
impl<Sel: ArraySelectionType> SetOperations for DatasetSelectionBase<Sel> {
    fn union(&self, other: &Self) -> Self {
        match (self, other) {
