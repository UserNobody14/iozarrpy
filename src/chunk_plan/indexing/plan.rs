...
pub enum GridJoinTree {
    /// Regular single grid with no dimensions or arrays of any other dimensionality to combine with:
    SingleGrid(OwnedGridGroup),
    /// Join two grid trees on certain arrays they have in common:
    JoinedGrid {
        left: Box<GridJoinTree>,
        right: Box<GridJoinTree>,
        // List of the dimensions they're joined on
        // In the most common case, this will be a single dimension (coordinate)
