...
use crate::chunk_plan::exprs::compile_ctx::{CompileCtx, LazyCompileCtx};
...
/// containing unresolved `ValueRange` constraints. Use `resolve_lazy_selection` to
/// resolve and materialize the result.
...
/// Complete lazy compilation workflow: compile -> resolve -> materialize.
///
/// This is equivalent to `compile_expr_to_dataset_selection` but uses the lazy
/// compilation path internally.
pub(crate) fn compile_expr_to_dataset_selection_lazy(
    expr: &Expr,
    meta: &ZarrDatasetMeta,
    store: zarrs::storage::ReadableWritableListableStorage,
    primary_var: &str,
...
/// Compile an expression to a chunk plan using the lazy compilation path.
pub(crate) fn compile_expr_to_chunk_plan_lazy(
    expr: &Expr,
    meta: &ZarrDatasetMeta,
    store: zarrs::storage::ReadableWritableListableStorage,
    primary_var: &str,
    ...
    let (selection, stats) =
        compile_expr_to_dataset_selection_lazy(expr, meta, store.clone(), primary_var)?;
...
use super::indexing::monotonic_async::AsyncMonotonicResolver;
use super::indexing::resolver_traits::AsyncCoordResolver;

/// Async resolution of a lazy selection.
///
/// Uses the async monotonic resolver for concurrent I/O.
pub(crate) async fn resolve_lazy_selection_async(
    ...
    // Count is not tracked for async resolver currently
    let stats = PlannerStats { coord_reads: 0 };

    Ok((selection, stats))
}

/// Async compile an expression to a dataset selection.
...
/// Async compile an expression to a chunk plan.
