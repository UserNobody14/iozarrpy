...
use super::{
    panic_to_py_err, to_py_err, ZarrSource,
};

impl ZarrSource {
    pub(super) fn try_set_predicate_impl(
        &mut self,
        ...
        let expr = std::panic::catch_unwind(
            std::panic::AssertUnwindSafe(|| {
                crate::py::expr_extract::extract_expr(
                    predicate,
                )
            }),
        )
    ...
    pub(super) fn set_with_columns_impl(
        &mut self,
        columns: Vec<String>,
    ) {
        self.with_columns = Some(
            columns
                .into_iter()
                .map(|s| s.istr())
                .collect(),
