...
pub(crate) fn _selected_variables_debug(
    ...
    let (selection, stats) =
        compile_expr_to_dataset_selection(&parsed_expr, &meta, opened.store.clone()).map_err(
            |e| match e {
    ...
    let inferred_vars: Vec<String> = selection
        .vars()
        .map(|(k, _)| k.to_string())
        .collect();
    ...
    let grouped_plan =
        crate::chunk_plan::selection_to_grouped_chunk_plan(&selection, &meta, opened.store.clone())
            .map_err(|e| match e {
                crate::chunk_plan::CompileError::Unsupported(e) => {
                    PyErr::new::<pyo3::exceptions::PyValueError, _>(e)
                }
    ...
    let mut out: HashMap<String, Vec<Py<PyAny>>> =
        HashMap::new();

    // Iterate over each variable's plan
    for (var_istr, sig) in
        grouped_plan.var_to_grid()
    {
        let var = var_istr.to_string();
        let Some(var_meta) =
            meta.arrays.get(var_istr)
        else {
            continue;
        };
        let arr = Array::open(
        ...
        let Some(subsets) =
            grouped_plan.by_grid().get(sig)
        else {
            continue;
        };

        // Convert array subsets to chunk indices
        let mut var_chunks: Vec<Py<PyAny>> =
            Vec::new();
        for subset in subsets.subsets_iter() {
            if let Ok(Some(chunks)) =
                arr.chunks_in_array_subset(subset)
            {
                for chunk_idx in
                    chunks.indices().iter()
                {
                    let idx: Vec<u64> = chunk_idx
                        .iter()
                        .copied()
                        .collect();
                    let chunk_shape_nz = arr.chunk_shape(&idx).map_err(|e| {
